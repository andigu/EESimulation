...
def start(position, angle, config):
    index = 0
    while not math.isnan(position) and not math.isnan(angle):
        position, angle = main.exit_status(position, angle, config) if index % 2 == 0 else main.enter_status(position,
                                                                                                             angle,
                                                                                                             config)
...
for i in range(1, 10000, 100):
    ...
    # file.write(str(i / 100) + " " + str(
    #     vary(lambda x, y: c.base * 0.1, lambda x, y: math.pi / 2 + math.pi / 2 * x / y, 900, c)) + "\n")
file.close()
print(now() - t)
